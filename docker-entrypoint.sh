...
exec chrome_driver & chromium-browser --headless --no-sandbox --hide-scrollbars --mute-audio --no-first-run \
    ...
    --disable-default-apps --disable-storage-reset --disable-dev-shm-usage --disable-sync \
    --disable-notifications --disable-accelerated-video-decode \
    --disable-client-side-phishing-detection --disable-setuid-sandbox \
    --disable-features=TranslateUI BlinkGenPropertyTrees --ignore-certificate-errors --no-default-browser-check \
    --metrics-recording-only --disable-threaded-animation
